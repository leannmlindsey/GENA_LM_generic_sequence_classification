...
in a single embedding pass — GENA-LM (bigbird / moderngena), for the
fragment-vs-genome-wide transfer analysis (best-head in the main table, full
per-head in the appendix).

Run ONCE PER VARIANT (the base model + embedding dir differ per variant):
    bigbird     --model_path AIRI-Institute/gena-lm-bigbird-base-t2t
    moderngena  --model_path AIRI-Institute/moderngena-base

For each genome-wide CSV: extract the base-model embeddings ONCE, then apply the
saved linear probe AND 3-layer NN (from embedding_analysis_gena_lm.py). Writes:
...
Usage:
    python genome_wide_all_heads_gena_lm.py \
        --model_path AIRI-Institute/gena-lm-bigbird-base-t2t \
        --embedding_dir <OUTPUT_DIR>/<w>/embedding/bigbird \
        --input_dir <LAMBDA_BASE>/genome_wide/<w> \
        --output_dir <OUTPUT_DIR>/<w>/genome_wide_heads/bigbird \
